...
## Dataset Relation
The analysis combines several datasets to 

estimate the impact of flood events on oil production in Poza Rica, Veracruz, Mexico within the Río Cazones basin. Precipitation data (ERA5), forecasted river flow (LISFLOOD), and historical flood events are combined in a regression model to descriptively analyze FIX. This will be combined with spacial data to better estimate the impact. In combining the spatial datasets (topography, hydrology, land-use, etc.), the hope is to improve understanding and differences of flood exposure to improve economic proxy for flood damages.
